...
legacy <- readOGR("./spatial_data/hakai_legacy_sites.gpkg")
legacy <- st_as_sf(legacy)
legacy$type <- "legacy"
...
unclassified_sites <- habitat_HIP(habitats = "unclassified", samples = 30, priority_habitats = c("giant_kelp", "bull_kelp", "seagrass"))
...
HIP_sites <- bind_rows(unclassified_sites, habitat_sites, high_rugosity_sites, low_rugosity_sites) %>% #add low and high rugosity sites here if you want to use the halton box method for them
  group_by(HaltonIndex) %>% 
  mutate(count = n()) %>% 
  arrange(desc(count), HaltonIndex)

selected_HIP <- st_centroid(HIP_sites)
selected_HIP$type <- "HIP"

#t1 <- selected_HIP %>%
#  group_by(HaltonIndex) %>%
#   summarise_each(funs(list(unique(habitat))))
...
all_sites <- bind_rows(selected_HIP, legacy)


area_plot+
  geom_sf(data = habitat_line_features, size = 0.3) +
  geom_sf(data = all_sites, aes(fill = factor(count)), size = 3, pch = 21)+
    scale_fill_brewer(palette = "Set1", name = "habitats\nin box")
ggsave("./figures/BAS_general_1.pdf", height = 8, width = 12)
